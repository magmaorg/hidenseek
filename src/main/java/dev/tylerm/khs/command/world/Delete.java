...
public class Delete implements ICommand {
    ...
    public void execute(Player sender, String[] args) {
        ...
        if (!worlds.contains(args[0])) {
            sender.sendMessage(
                    Config.errorPrefix
                            + Localization.message("WORLD_DOESNT_EXIST").addAmount(args[0]));
            return;
        }

        Confirm.Confirmation confirmation =
                new Confirm.Confirmation(
                        args[0],
                        world -> {
                            ...
                            if (!worlds_now.contains(world)) {
                                sender.sendMessage(
                                        Config.errorPrefix
                                                + Localization.message("WORLD_DOESNT_EXIST")
                                                        .addAmount(world));
                                return;
                            }
                            ...
                            if (bukkit_world != null && bukkit_world.getPlayers().size() > 0) {
                                sender.sendMessage(
                                        Config.errorPrefix
                                                + Localization.message("WORLD_NOT_EMPTY"));
                                return;
                            }
                            String path =
                                    Main.getInstance().getWorldContainer().getPath()
                                            + File.separator
                                            + world;
                            if (!Bukkit.getServer().unloadWorld(world, false)) {
                                sender.sendMessage(
                                        Config.errorPrefix
                                                + Localization.message("WORLD_REMOVED_FAILED"));
                                return;
                            }
                            try {
                            ...
                            sender.sendMessage(
                                    Config.messagePrefix
                                            + Localization.message("WORLD_REMOVED")
                                                    .addAmount(world));
                        });
