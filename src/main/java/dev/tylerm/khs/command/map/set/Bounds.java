...
public class Bounds implements ICommand {
    ...
    public void execute(Player sender, String[] args) {
        ...
        if (map.getSeekerLobby().isNotSetup()) {
            sender.sendMessage(
                    Config.errorPrefix + Localization.message("ERROR_GAME_SEEKER_SPAWN"));
            return;
        }
        ...
        if (!map.isBoundsNotSetup()) {
            if (!map.getSpawn().isNotSetup()) {
                if (map.getSpawn().isNotInBounds(bxs, bxl, bzs, bzl)) {
                    ...
                    sender.sendMessage(
                            Config.warningPrefix + Localization.message("WARN_SPAWN_RESET"));
                }
            }
            if (!map.getSeekerLobby().isNotSetup()) {
                if (map.getSeekerLobby().isNotInBounds(bxs, bxl, bzs, bzl)) {
                    ...
                    sender.sendMessage(
                            Config.warningPrefix + Localization.message("WARN_SEEKER_SPAWN_RESET"));
                }
            }
        }
        Maps.setMap(map.getName(), map);
        sender.sendMessage(
                Config.messagePrefix + Localization.message("BOUNDS").addAmount(first ? 1 : 2));
    }
