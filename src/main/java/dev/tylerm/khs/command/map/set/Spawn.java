                ...
                map -> {
                    if (map.isWorldBorderEnabled()
                            && new Vector(
                                            ...
                                            .distance(map.getWorldBorderPos())
                                    > 100) {
                        sender.sendMessage(
                                Config.errorPrefix + Localization.message("WORLDBORDER_POSITION"));
                        throw new RuntimeException(
                                "World border not enabled or not in valid position!");
                    }
                    ...
                    if (!map.isBoundsNotSetup()) {
                        ...
                        if (map.getSpawn()
                                .isNotInBounds(
                                        ...
                                        boundsMax.getBlockZ())) {
                            sender.sendMessage(
                                    Config.warningPrefix + Localization.message("WARN_MAP_BOUNDS"));
                        }
                    }
                    ...
                    if (map.getSeekerLobby().getWorld() != null
                            && !map.getSeekerLobby()
                                    .getWorld()
                                    .equals(sender.getLocation().getWorld().getName())) {
                        sender.sendMessage(
                                Config.warningPrefix
                                        + Localization.message("SEEKER_LOBBY_SPAWN_RESET"));
                        map.setSeekerLobby(Location.getDefault());
                    }

                    if (!sender.getLocation().getWorld().getName().equals(map.getSpawnName())
