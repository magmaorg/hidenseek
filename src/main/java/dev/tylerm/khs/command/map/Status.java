...
public class Status implements ICommand {
    ...
    public void execute(Player sender, String[] args) {
        ...
        if (map.getSpawn().getBlockX() == 0
                        && map.getSpawn().getBlockY() == 0
                        && map.getSpawn().getBlockZ() == 0
                || !map.getSpawn().exists()) {
            msg = msg + "\n" + Localization.message("SETUP_GAME");
            count++;
        }
        if (map.getLobby().getBlockX() == 0
                        && map.getLobby().getBlockY() == 0
                        && map.getLobby().getBlockZ() == 0
                || !map.getLobby().exists()) {
            msg = msg + "\n" + Localization.message("SETUP_LOBBY");
            count++;
        }
        if (map.getSeekerLobby().getBlockX() == 0
                        && map.getSeekerLobby().getBlockY() == 0
                        && map.getSeekerLobby().getBlockZ() == 0
                || !map.getSeekerLobby().exists()) {
            msg = msg + "\n" + Localization.message("SETUP_SEEKER_LOBBY");
            count++;
        }
        if (Config.exitPosition.getBlockX() == 0
                        && Config.exitPosition.getBlockY() == 0
                        && Config.exitPosition.getBlockZ() == 0
                || !Config.exitPosition.exists()) {
