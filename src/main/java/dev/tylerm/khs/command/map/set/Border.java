...
public class Border implements ICommand {
    public void execute(Player sender, String[] args) {
        ...
        try {
            ...
            sender.sendMessage(
                    Config.errorPrefix
                            + Localization.message("WORLDBORDER_INVALID_INPUT").addAmount(args[0]));
            return;
        }
        try {
            ...
            sender.sendMessage(
                    Config.errorPrefix
                            + Localization.message("WORLDBORDER_INVALID_INPUT").addAmount(args[1]));
            return;
        }
        try {
            ...
            sender.sendMessage(
                    Config.errorPrefix
                            + Localization.message("WORLDBORDER_INVALID_INPUT").addAmount(args[2]));
            return;
        }
        if (num < 100) {
        ...
        if (change < 1) {
            sender.sendMessage(
                    Config.errorPrefix + Localization.message("WORLDBORDER_CHANGE_SIZE"));
            return;
        }
        ...
        sender.sendMessage(
                Config.messagePrefix
                        + Localization.message("WORLDBORDER_ENABLE")
                                .addAmount(num)
                                .addAmount(delay)
                                .addAmount(change));
