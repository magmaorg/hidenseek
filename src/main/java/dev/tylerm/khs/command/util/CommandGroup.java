...
public class CommandGroup {
    ...
    public void handleCommand(Player player, String[] args) {
        ...
        if (data == null) {
            player.sendMessage(
                    String.format(
                                    "%s%sKenshin's Hide and Seek %s(%s1.7.6%s)\n",
                                    ChatColor.AQUA,
                                    ChatColor.BOLD,
                                    ChatColor.GRAY,
                                    ChatColor.WHITE,
                                    ChatColor.GRAY)
                            + String.format(
                                    "%sAuthor: %s[KenshinEto]\n", ChatColor.GRAY, ChatColor.WHITE)
                            + String.format(
                                    "%sHelp Command: %s/hs %shelp",
                                    ChatColor.GRAY, ChatColor.AQUA, ChatColor.WHITE));
            return;
        }
        ...
        int parameterCount =
                (int)
                        Arrays.stream(command.getUsage().split(" "))
        ...
        if (args.length == 1) {
            ...
            if (commandRegister.containsKey(invoke)) {
                ...
                if (object instanceof CommandGroup) {
                    CommandGroup group = (CommandGroup) object;
                    return group.handleTabComplete(
                            player,
                            permission + "." + group.getLabel(),
                            Arrays.copyOfRange(args, 1, args.length));
