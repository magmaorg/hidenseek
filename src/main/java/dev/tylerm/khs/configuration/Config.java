...
public class Config {
    ...
    public static int minPlayers,
            ...
            hidingTimer,
            endGameDelay;
    public static float seekerPingLeadingVolume, seekerPingVolume, seekerPingPitch;
    public static List<String> blockedCommands, blockedInteracts;
    public static ItemStack lobbyLeaveItem,
            lobbyStartItem,
            glowPowerupItem,
            flightToggleItem,
            teleportItem;
    public static XSound ringingSound, heartbeatSound;
    ...
    public static void loadConfig() {
        ...
        minPlayers =
                Math.max(
                        1 + startingSeekerCount + (waitTillNoneLeft ? 0 : 1),
        ...
        Optional<XSound> heartbeatOptional =
                XSound.matchXSound(config.getString("seekerPing.sounds.heartbeatNoise"));
        heartbeatSound = heartbeatOptional.orElse(XSound.BLOCK_NOTE_BLOCK_BASEDRUM);
        Optional<XSound> ringingOptional =
                XSound.matchXSound(config.getString("seekerPing.sounds.ringingNoise"));
        ...
        try {
            ...
            throw new RuntimeException(
                    "hideCountdownDisplay: "
                            + config.getString("hideCountdownDisplay")
        ...
        bungeeLeave =
                config.getString("leaveType") == null
                        || config.getString("leaveType").equalsIgnoreCase("proxy");
        ...
        if (!databaseType.equals("SQLITE") && !databaseType.equals("MYSQL")) {
            throw new RuntimeException(
                    "databaseType: " + databaseType + " is not a valid configuration option!");
        }
