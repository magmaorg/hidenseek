...
public class Game {
    ...
    public void start(List<Player> seekers) {
        ...
        board.reload();
        board.setInitialSeekers(
                seekers.stream().map(Player::getUniqueId).collect(Collectors.toList()));
        seekers.forEach(
                seeker -> {
    ...
    private void stop(WinType type) {
        ...
        List<UUID> players =
                board.getPlayers().stream().map(Entity::getUniqueId).collect(Collectors.toList());
        if (type == WinType.HIDER_WIN) {
            List<UUID> winners =
                    board.getHiders().stream()
            ...
            Main.getInstance()
                    ...
                    .addWins(
                            ...
                            winners,
                            board.getHiderKills(),
                            board.getHiderDeaths(),
                            board.getSeekerKills(),
                            board.getSeekerDeaths(),
                            type);
        } else if (type == WinType.SEEKER_WIN) {
            List<UUID> winners = new ArrayList<>();
            board.getInitialSeekers()
                    .forEach(
                            p -> {
        ...
        Bukkit.getScheduler()
                .scheduleSyncDelayedTask(Main.getInstance(), this::end, endGameDelay * 20);
    }
        ...
        if (status != Status.STARTING && status != Status.PLAYING) {
            if (saveInventory) {
                ...
                Main.getInstance()
                        .getDatabase()
                        .getInventoryData()
                        .saveInventory(player.getUniqueId(), data);
            }
            PlayerLoader.joinPlayer(player, currentMap);
            board.addHider(player);
            board.createLobbyBoard(player);
        ...
        if (saveInventory) {
            ItemStack[] data =
                    Main.getInstance()
                            .getDatabase()
                            .getInventoryData()
                            .getInventory(player.getUniqueId());
            try {
    ...
    private void whileWaiting() {
        ...
        if (lobbyMin <= board.size()) {
            if (lobbyTimer < 0) lobbyTimer = countdown;
            if (board.size() >= changeCountdown) lobbyTimer = Math.min(lobbyTimer, 10);
            if (gameTick % 20 == 0) {
                lobbyTimer--;
                board.reloadLobbyBoards();
            }
    ...
    private void checkWinConditions() {
        ...
        if (hiderCount < 1 || (!waitTillNoneLeft && hiderCount < 2)) {
            if (hiderLeft && dontRewardQuit) {
                ...
                if (gameOverTitle)
                    broadcastTitle(
                            message("GAME_TITLE_NO_WIN").toString(),
                ...
                if (hiderCount < 1 || waitTillNoneLeft) {
                    if (announceMessagesToNonPlayers)
                        Bukkit.broadcastMessage(
                                gameOverPrefix + message("GAME_GAMEOVER_HIDERS_FOUND"));
                    else broadcastMessage(gameOverPrefix + message("GAME_GAMEOVER_HIDERS_FOUND"));
                    if (gameOverTitle)
                        broadcastTitle(
                                message("GAME_TITLE_SEEKERS_WIN").toString(),
                    ...
                    if (announceMessagesToNonPlayers)
                        Bukkit.broadcastMessage(
                                gameOverPrefix
                                        + message("GAME_GAMEOVER_LAST_HIDER").addPlayer(hider));
                    else
                        broadcastMessage(
                                gameOverPrefix
                                        + message("GAME_GAMEOVER_LAST_HIDER").addPlayer(hider));
                    if (gameOverTitle)
                        broadcastTitle(
                                message("GAME_TITLE_SINGLE_HIDER_WIN").addPlayer(hider).toString(),
                                message("GAME_SUBTITLE_SINGLE_HIDER_WIN")
                                        .addPlayer(hider)
                                        .toString());
                }
            ...
            if (gameOverTitle)
                broadcastTitle(
                        message("GAME_TITLE_HIDERS_WIN").toString(),
                        message("GAME_GAMEOVER_TIME").toString());
